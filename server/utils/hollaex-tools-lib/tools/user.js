		...
		.then(async (user) => {
			if (user && networkData) {
				...
				try {
					...
					if (networkData && networkData.balance && Object.keys(networkData.balance).length > 0) {
						balance = networkData.balance;
						wallet = networkData.wallet || [];
					}
				} catch (e) {}
				if (Object.keys(balance).length === 0) {
					const result = await getModel('sequelize').query(
						'SELECT * FROM user_wallets WHERE user_id = :userId',
						{ replacements: { userId: user.id }, type: getModel('sequelize').QueryTypes.SELECT }
					);
					for (const uw of result) {
						...
						wallet.push({ currency: uw.currency, address: uw.address || '', network: uw.network || '' });
					}
				}
...
const createUserCryptoAddressByKitId = async (kitId, crypto, opts = {
	...
	const { generateWallet } = require('../../../wallets/generate');
	const result = await generateWallet(kitId, crypto, opts ? opts.network : null);

	const idDictionary = await mapKitIdToNetworkId([kitId]);
	if (!has(idDictionary, kitId)) {
		throw new Error(USER_NOT_FOUND);
	}
	const network_id = idDictionary[kitId];

	const user = await getModel('user').findOne({ where: { id: kitId } });
	if (!user) throw new Error(USER_NOT_FOUND);

	const crypto_wallet = user.crypto_wallet || {};
	if (!crypto_wallet[crypto]) {
		crypto_wallet[crypto] = {};
	}
	crypto_wallet[crypto][opts?.network || 'main'] = result.address;

	await getModel('sequelize').query(
		'UPDATE users SET crypto_wallet = crypto_wallet || :wallet::jsonb WHERE id = :id',
		{ replacements: { wallet: JSON.stringify(crypto_wallet), id: kitId } }
	);

	try {
		const network_name = opts?.network || 'main';
		await getModel('sequelize').query(
			`INSERT INTO user_wallets (user_id, currency, balance, available, address, network, is_valid, created_at, updated_at)
			 VALUES (:userId, :currency, 0, 0, :address, :network, true, NOW(), NOW())
			 ON CONFLICT (user_id, currency) DO UPDATE SET
			   address = EXCLUDED.address,
			   network = EXCLUDED.network,
			   updated_at = NOW()`,
			{ replacements: { userId: kitId, currency: crypto, address: result.address, network: network_name } }
		);
	} catch (e) {
		console.error('Failed to sync user_wallets:', e.message);
	}
	...
	return {
		...
		crypto: crypto,
		network: opts ? opts.network : null,
		...result
	};
};
