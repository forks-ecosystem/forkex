...
const { has, isPlainObject, isBoolean, isNumber, isString } = require('lodash');
...
loggerOrders.error('=====62 symbol:', symbol);
	loggerOrders.debug(
	...
	if (opts.discount) {
		loggerOrders.debug(
			'h ollaex-tools-lib/order/generateOrderFeeData discount',
			'discount percentage',
			opts.discount
		);
		...
		const exchangeMinFee = getMinFees() || { maker: 0, taker: 0 };
		loggerOrders.debug(
