...
const verifyHmacTokenMiddleware = (req, definition, apiKey, cb, isSocket = false) => {
	...
	if (has(req.headers, 'api-key') && has(req.headers, 'authorization')) {
		...
		verifyHmacTokenPromise(
			...
			.catch((err) => {
				loggerAuth.error('1.helpers/auth/checkApiKey catch', err);
				return sendError(err.message);
			});
	}
};
...
const verifyHmacTokenPromise = (apiKey, apiSignature, apiExpires, method, originalUrl, body, permissions = [], ip = undefined) => {
		...
		return findTokenByApiKey(apiKey)
			.then((token) => {
				if (token.whitelisting_enabled && token.whitelisted_ips.length > 0) {
					...
					if (!found) {
						loggerAuth.error(
							'2.helpers/auth/checkApiKey/findTokenByApiKey not whitelisted',
							apiKey,
				...
				if (new Date(token.expiry) < new Date()) {
					loggerAuth.error(
						'3.helpers/auth/checkApiKey/findTokenByApiKey expired key',
						apiKey
					);
					throw new Error(API_KEY_EXPIRED);
				} else if (!token.active) {
					loggerAuth.error(
						'4.helpers/auth/checkApiKey/findTokenByApiKey inactive',
						apiKey
					);
					throw new Error(API_KEY_INACTIVE);
} else if (permissions.length > 0) {
    const checkPerm = (perm) => {
        if (token.permission && token.permission[perm] === true) return true;
        if (token.permissions && token.permissions[perm] === true) return true;
        if (token[perm] === true) return true;
        if (perm === 'can_read' && (
            (token.permission && token.permission.read === true) ||
            (token.permissions && token.permissions.read === true) ||
            token.read === true
        )) return true;
        return false;
    };
    ...
    if (!hasAllPermissions) {
        loggerAuth.error(
            '5.helpers/auth/checkApiKey/findTokenByApiKey not permitted',
            apiKey,
            permissions,
            'Token permissions:',
            token.permissions || token.permission
        );
    ...
    return {
        sub: { id: token.user_id, email: token.email, networkId: 1 },
        scopes: [token.role]
    };
} else {
    const isSignatureValid = checkHmacSignature(
