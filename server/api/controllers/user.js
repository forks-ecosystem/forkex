...
const signUpUser = (req, res) => {
	...
	toolsLib.security.checkIp(ip)
		...
		.catch((err) => {
			const lang = req?.auth?.sub?.lang || toolsLib.getKitConfig().defaults.language;
			loggerUser.error(req.uuid, 'controllers/user/signUpUser', err.message);
			const messageObj = errorMessageConverter(err, lang);
...
const loginPost = (req, res) => {
	...
	toolsLib.security.checkIp(ip)
		...
		.catch((err) => {
			...
			return res.status(err.statusCode || 401).json({ message: errorMessageConverter(err, lang)?.message, lang: errorMessageConverter(err, lang)?.lang, code: errorMessageConverter(err, lang)?.code });
		});
};
...
const loginWithGoogle = async (req, res) => {
	...
	try {
		...
		return res.status(err.statusCode || 401).json({ 
			message: errorMessageConverter(err, lang)?.message, 
			lang: errorMessageConverter(err, lang)?.lang, 
			code: errorMessageConverter(err, lang)?.code 
		});
	}
};
...
const getUser = (req, res) => {
	...
	toolsLib.user.getUserByKitId(id, true, false, {
		additionalHeaders: {
		...
		.then(async (user) => {
			...
			// Read balance directly from 'balances' table (source of truth)
			try {
				...
				balances
					...
					.forEach(b => {
						balanceObj[b.currency + '_balance'] = Number(b.balance);
						balanceObj[b.currency + '_available'] = Number(b.available);
						balanceObj[b.currency + '_pending'] = Number(b.balance) - Number(b.available);
					});
				user.balance = balanceObj;
			} catch (e) {}
			const walletArr = [];
			if (user.crypto_wallet && typeof user.crypto_wallet === 'object') {
				for (const [currency, networks] of Object.entries(user.crypto_wallet)) {
					const coinConfig = toolsLib.getKitCoinsConfig();
					if (!coinConfig[currency]) continue;
					if (networks && typeof networks === 'object') {
						for (const [network, address] of Object.entries(networks)) {
							if (address) {
								walletArr.push({ currency, address, network });
							}
						}
					}
				}
			}
			const userData = toolsLib.user.omitUserFields(user);
			if (user.balance) userData.balance = user.balance;
			userData.wallet = walletArr;
...
const getUserBalance = async (req, res) => {
	...
	try {
		...
		loggerUser.error(req.uuid, 'controllers/user/getUserBalance (fallback)', err.message);
		try {
			const sequelize = require('/app/db/models').sequelize;
			const result = await sequelize.query(
				'SELECT currency, balance, available FROM user_wallets WHERE user_id = :userId',
				{ replacements: { userId: user_id }, type: sequelize.QueryTypes.SELECT }
			);
			const balance = {};
			for (const uw of result) {
				balance[uw.currency + '_balance'] = Number(uw.balance) || 0;
				balance[uw.currency + '_available'] = Number(uw.available) || 0;
			}
			return res.json(balance);
		} catch (e2) {
			return res.json({});
...
const fetchUserProfitLossInfo = (req, res) => {
	...
	toolsLib.user.fetchUserProfitLossInfo(user_id, { period: period.value || 7 })
		...
		.catch((err) => {
			...
			return res.json({ balance: {}, total: 0, user_id: user_id, history: [] });
		});
};
...
const getUserWalletAddresses = async (req, res) => {
	const user_id = req.auth.sub.id;
	const { currency, network } = req.swagger.params;
	try {
		const sequelize = require('/app/db/models').sequelize;
		let query = 'SELECT currency, address, network FROM user_wallets WHERE user_id = :userId AND address IS NOT NULL AND address != \'\'';
		const replacements = { userId: user_id };
		if (currency && currency.value) {
			query += ' AND currency = :currency';
			replacements.currency = currency.value;
		}
		if (network && network.value) {
			query += ' AND network = :network';
			replacements.network = network.value;
		}
		console.log('getUserWalletAddresses query:', query, replacements);
		const result = await sequelize.query(query, { replacements, type: sequelize.QueryTypes.SELECT });
		console.log('getUserWalletAddresses result:', JSON.stringify(result));
		const addresses = {};
		for (const row of result) {
			if (!addresses[row.currency]) {
				addresses[row.currency] = {};
			}
			addresses[row.currency][row.network || 'main'] = row.address;
		}
		return res.json(addresses);
	} catch (err) {
		loggerUser.error(req.uuid, 'controllers/user/getUserWalletAddresses', err.message);
		return res.json({});
	}
};
...
const getUserWallet = (req, res) => {
	...
	toolsLib.user.getUserByKitId(user_id)
		.then((user) => {
			const crypto_wallet = user.crypto_wallet || {};
			const wallet = [];
			const coinConfig = toolsLib.getKitCoinsConfig();
			for (const [currency, networks] of Object.entries(crypto_wallet)) {
				if (!coinConfig[currency]) continue;
				if (networks && typeof networks === "object") {
					for (const [network, address] of Object.entries(networks)) {
						if (address) {
							wallet.push({ currency, address, network });
						}
				}
			}
			}
		...
		.catch((err) => {
			loggerUser.error(req.uuid, "controllers/user/getUserWallet", err.message);
			return res.json([]);
		});
};
...
const getUserSettings = (req, res) => {
	...
	toolsLib.user.getUserByKitId(user_id)
		...
		.catch((err) => {
			loggerUser.error(req.uuid, "controllers/user/getUserSettings", err.message);
			return res.json({});
		});
};
module.exports = {
