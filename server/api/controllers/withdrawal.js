...
const getWithdrawalMax = (req, res) => {
	...
	toolsLib.wallet.calculateWithdrawalMax(
		...
		.catch(async (err) => {
			loggerWithdrawals.error(req.uuid, 'controllers/withdrawal/getWithdrawalMax (fallback)', err.message);
			try {
				const sequelize = require('/app/db/models').sequelize;
				const result = await sequelize.query(
					'SELECT available FROM user_wallets WHERE user_id = :userId AND currency = :currency LIMIT 1',
					{ replacements: { userId: req.auth.sub.id, currency: currency.value }, type: sequelize.QueryTypes.SELECT }
				);
				const available = result.length > 0 ? Number(result[0].available) : 0;
				return res.json({ amount: available });
			} catch (e2) {
