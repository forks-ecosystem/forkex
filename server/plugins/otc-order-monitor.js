...
const checkOtcOpenOrders = async () => {
	try {
		...
		const res = await toolsLib.order.getAllExchangeOrders(null, null, null, true, null, null, 'created_at', 'desc', null, null, null, {});
		const orders = (res && res.data) || [];
		loggerPlugin.verbose('1.plugins/otc-order-monitor checkOtcOpenOrders length', orders.length);
		...
		loggerPlugin.verbose('2.plugins/otc-order-monitor checkOtcOpenOrders', Object.keys(otcOpenOrdersBySymbol));
		...
		for (const symbol of Object.keys(otcOpenOrdersBySymbol)) {
			try {
				...
				loggerPlugin.error('3.plugins/otc-order-monitor refresh check error', symbol, err.message);
			}
		}
	} catch (err) {
		loggerPlugin.error('4.plugins/otc-order-monitor checkOtcOpenOrders error', err.message);
	}
};
