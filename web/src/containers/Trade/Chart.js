...
class TVChartContainer extends React.PureComponent {
	static defaultProps = {
		symbol: 'lbtc-usdt', // the trading tab we are in should be passed here
		interval: '60',
	...
	UNSAFE_componentWillMount() {
		...
		this.chartConfig = {
			...
			subscribeBars: (
				...
				if (that._chartPollInterval) clearInterval(that._chartPollInterval);
				that._lastTradeId = null;
				that._chartPollInterval = setInterval(() => {
					fetch(`/v2/trades?symbol=${symbolInfo.ticker}&limit=1`)
						.then(r => r.json())
						.then(resp => {
							const trades = resp && resp.data ? resp.data : (Array.isArray(resp) ? resp : null);
							if (trades && trades.length && trades[0]) {
								const trade = trades[0];
								if (trade.id === that._lastTradeId) return;
								that._lastTradeId = trade.id;
								const sub = that.state.sub;
								if (sub) {
									that.updateBar({
						...
						.catch(() => {});
					try { if (that.tvWidget) that.tvWidget.chart().refresh(); } catch(e) {}
				}, 10000);
			},
			unsubscribeBars: (subscriberUID) => {
				if (that._chartPollInterval) {
					clearInterval(that._chartPollInterval);
					that._chartPollInterval = null;
				}
			},
	...
	componentWillUnmount() {
		if (this._chartPollInterval) {
			clearInterval(this._chartPollInterval);
			this._chartPollInterval = null;
		}
