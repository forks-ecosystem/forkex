...
import { setOrderbooks, setTrades } from 'actions/orderbookAction';
...
import { getUserTrades, addUserTrades } from 'actions/walletActions';
...
class Trade extends PureComponent {
	constructor(props) {
		...
		this._orderbookPollInterval = null;
		this._tradesPollInterval = null;
	}
	...
	componentWillUnmount() {
		...
		if (this._orderbookPollInterval) {
			clearInterval(this._orderbookPollInterval);
			this._orderbookPollInterval = null;
		}
		if (this._tradesPollInterval) {
			clearInterval(this._tradesPollInterval);
			this._tradesPollInterval = null;
		}
	...
	initializeOrderbookWs = (symbol, token = '') => {
		...
		// REST fallback: poll orderbook every 5s
		if (this._orderbookPollInterval) clearInterval(this._orderbookPollInterval);
		this._orderbookPollInterval = setInterval(() => {
			const pair = this.props.routeParams.pair;
			if (!pair) return;
			fetch(`/v2/orderbook?symbol=${pair}`)
				.then(r => r.json())
				.then(data => {
					if (data && data.asks && data.bids) {
						const flat = [
							...(data.bids || []).map(b => [b[0], b[1], 0]),
							...(data.asks || []).map(a => [a[0], a[1], 0]),
						];
						this.props.setOrderbooks({
							[pair]: flat,
						});
						if (!this.state.orderbookFetched) {
							this.setState({ orderbookFetched: true });
						}
					}
				})
		...
		// REST fallback: poll public trades every 5s for Recent Trades panel
		if (this._tradesPollInterval) clearInterval(this._tradesPollInterval);
		this._tradesPollInterval = setInterval(() => {
			const pair = this.props.routeParams.pair;
			if (!pair) return;
			fetch(`/v2/trades?symbol=${pair}&limit=50`)
				.then(r => r.json())
				.then(data => {
					const trades = Array.isArray(data) ? data : (data && data.data ? data.data : []);
					if (trades.length) {
						this.props.addUserTrades(trades);
					}
				})
				.catch(() => {});
		}, 5000);
